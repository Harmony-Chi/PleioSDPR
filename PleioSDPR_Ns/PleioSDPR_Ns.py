...
ref_ld_mat1, ref_ld_mat2,ref_ld_mat_, ID_trait1 , ID_trait2 , s,mcmc_samples,     burn,  n_threads, chr):
    M = [1, 1000,1000,1000]
    ...
    state['a'] = 0.1; state['c'] = 1; state['ovp_pcor']=s
    print(state['ovp_pcor'])
    print('ovp_pcor')
...
def phen_ovp(N1,N2,Ns,ss1,ss2,load_ld,chr=10):
    all_refs=[]
    nblock=0
    for i in range(chr):
        f = gzip.open(load_ld + '/chr_' + str(i+1) +'.gz', 'r')
        try:
        ...
        ld = ld_dict[4]
        ldscores = []
        nblock=nblock+len(ld)
        for block in ld:
            for i in range(len(block)):
                sum_square = sum(block[i]**2)
                ldscores.append(sum_square)
        ref = pd.DataFrame({'SNP':snps, 'A1':a1, 'A2':a2,'ldscore':ldscores})
        all_refs.append(ref)
    final_ref=pd.concat(all_refs,ignore_index=True)
    ss1_=pd.merge(ss1,final_ref,on="SNP")
    ss2_=pd.merge(ss2,final_ref,on="SNP")
    idx_ss1=np.logical_and(ss1_['A1_x']==ss1_['A2_y'],ss1_['A2_x']==ss1_["A1_y"])
    idx_ss2=np.logical_and(ss2_['A1_x']==ss2_['A2_y'],ss2_['A2_x']==ss2_["A1_y"])
    ss1_[idx_ss1]['Z']=-ss1_[idx_ss1]['Z']
    ss2_[idx_ss2]['Z']=-ss2_[idx_ss2]['Z']
    h1=(np.mean(ss1_['Z'] ** 2) - 1) / np.mean(ss1_['ldscore']) * ss1.shape[0] / N1
    h2=(np.mean(ss2_['Z'] ** 2) - 1) / np.mean(ss2_['ldscore']) * ss2.shape[0] / N2
    ss1_=ss1_[["SNP","Z","ldscore"]]
    ss2_=ss2_[["SNP","Z"]]
    ss=pd.merge(ss1_,ss2_,on="SNP")
    m = ss.shape[0]
    Z_x = ss['Z_x']
    Z_y = ss['Z_y']
    ldscores = ss['ldscore']
    ...
    weight1 = 1 + N1*h1*ldscores/m
    weight2 = 1 + N2*h2*ldscores/m
    weight3=np.mean(Z_xy)*ldscores
    weight=1/(weight1*weight2+weight3**2)

    blocksize=int(np.floor(m/nblock))+1
    q_block_slope = np.empty(nblock)
    q_block_intercept = np.empty(nblock)

    for j in range(nblock):
        ind_start = blocksize*j
        ind_end = min(m-1, blocksize * (i + 1))
        l = np.concatenate([ldscores[:ind_start], ldscores[ind_end:]], axis=0)
        z_xy = np.concatenate([Z_xy[:ind_start], Z_xy[ind_end:]], axis=0)
        w = np.concatenate([weight[:ind_start], weight[ind_end:]], axis=0)
        lm = linear_model.LinearRegression().fit(pd.DataFrame(l), pd.DataFrame(z_xy), sample_weight=w)
        q_block_intercept[j] = lm.intercept_[0]
        q_block_slope[j]=lm.coef_[0]

    gcov=np.mean(q_block_slope)*m/np.sqrt(N1*N2)
    s=(np.mean(q_block_intercept)*np.sqrt(N1*N2)-Ns*gcov)/np.sqrt(N1*N2)
    return s


def pipeline(args):
    
    N1 = args.N1; N2 = args.N2; Ns=args.Ns
    ...
    if Ns==0:
        ...
        s=phen_ovp(N1,N2,Ns,ss1,ss2,args.load_ld,chr=22)
    ...
    res1, res2 = PleioSDPR_gibbs(beta_margin1=np.array(beta_margin1)/args.c1, beta_margin2=np.array(beta_margin2)/args.c2, 
                             ...
                             ref_ld_mat1=ref_ld_mat1, ref_ld_mat2=ref_ld_mat2, ref_ld_mat_=ref_ld_mat_, ID_trait1 = ID_trait1, ID_trait2 = ID_trait2, s=s,mcmc_samples=args.mcmc_samples, 
...
parser.add_argument('--Ns', type=int, default=None, required=True,
                        help='Number of individuals overlapped in two summary statistic.')
